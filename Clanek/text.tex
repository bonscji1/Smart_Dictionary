...
\begin{abstract}
        ...
        Bohužel se ukázalo, že volně dostupné prostředky pro digitalizaci nejsou dostatečně vyspělé.
        Zatím s jejich použitím nebylo možné dosáhnout představeného cíle.
...
Existují velice jasné návaznosti, kdy novější studie upřesňuje výsledky starších studií nebo doplňují starší studii o novější  technologie.
...
Nelze s jistotou říci, zda je tato skutečnost způsobena jejich schopností pracovat s moderními technologiemi nebo jejich znalostní bází anglického jazyka, který je v této studii testován.
...
Za pomoci mobilního telefonu je možné studovat kdekoli a to v porovnání se studiem na stolních počítačích ve třídě vedlo k lepším výsledkům.
...
Nejčastější je využití Smart pera, smartphonu nebo samostatné aplikace.
...
Systém využívá  3 propojené části, již zmíněné Smart pero, speciální tečkovaný blok a dedikovanou aplikaci.
...
Ne každý uživatel je ochotný zaplatit požadované peněžní částky za takto specifický use-case.\cite{popupalr_science_digitalization_with_pens}
...
Je však možné vyřešit tento problém pomocí specializované aplikace.
Například aplikace Google Keep umožňuje z obrázku extrahovat čistý text, podobně jako aplikace Live Text.\cite{aarp_digitalization, google_keep}
...
Posledním bodem nutným zmínit jsou data.
Nové řešení bude tvořeno s využitím dat získaných ze studia španělštiny.
...
Tato technologie není na rozdíl od technologií Tesseract a EasyOCR využitelná pro praktické provedení nového řešení.
...
\ac{OCR} řešení původně vyvíjené v Hewlett-packard laboratořích v roce 1985, později vydané jako \textit{open-source} v roce 2005.
...
\subsubsection{EasyOCR}
 \textit{Open-source} python modul pro extrakci textu z obrázků.
...
Komerční řešení postavené zcela na myšlence převodu ručně psaného textu do digitální podoby.
...
Vrácený výsledek také není plně uspokojivý, více v sekci \textit{Testování aplikace}.
...
Z tohoto důvodu se  sekce \textit{Testování aplikace} bude věnovat testování limitů již dříve zmíněných řešení a jejich porovnáním.
...
Z obrázku 1 je jasné, že všechna řešení dokázala extrahovat text ze vstupního testovacího obrázku (a).
Výstupy se od sebe ale značně liší.
U EasyOCR dokonce došlo k přehození slov.
Všechna řešení však digitalizovala text v dostatečné kvalitě pro pokračování implementace.
